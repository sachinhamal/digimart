                    ...
                    <div class="card-header" data-background-color="custom-color">
                        <div class="row">
                            ...
                            <div class="col-md-6 text-right">
                                <a class="btn btn-primary btn-sm" href="{{route('blog.create')}}">Add
                                    New
                                </a>
                            </div>
                        </div>
                            ...
                            @if($blog->count()>0)
                                @foreach($blog as $product)
                                    <tr>
                                        ...
                                        <td><img src="{{url('images/blog/'.$product->image)}}" width="200px" alt="{{$product->title}}"></td>
                                        <td><form action="{{route('blog.status',$product->id)}}" method="POST">
                                        @csrf
                                        ...
                                        <td class="td-actions text-left">
                                            ...
                                            <a href="{{route('blog.show',$product->id)}}">
                                                <button type="button" class="btn btn-outline-info">Details</button>
                                            </a>
                                            <a href="{{route('blog.edit',$product->id)}}">
                                                <button type="button" class="btn btn-outline-warning">Edit</button>
                                            </a>
                                            <a type="button" onClick="javascript: return confirm('Please confirm deletion');" href="{{route('blog.delete',$product->id)}}" data-id="{{ $product->id }}"
